...
class Customers(MSDBconnection):
    ...
    def get_by_company(self, company_name):
        name_result_table = self.sql_query(f"SELECT * FROM Customers WHERE CompanyName LIKE '%{company_name}%'")
        if name_result_table.fetchone() is None:
            print(f'No results matching "{company_name}".')
        while True:
...
client_table.get_by_company('about')
